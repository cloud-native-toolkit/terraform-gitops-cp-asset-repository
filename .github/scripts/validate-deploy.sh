...
ASSETREPO_CRD="assetrepository"
TIMEOUT=60
count=0
DESIRED_STATE="Ready"

until [[ $(kubectl get ${ASSETREPO_CRD}  -n  ${NAMESPACE} -o jsonpath="{range .items[*]}{.status.phase}{end}") == ${DESIRED_STATE} ||  $count -eq ${TIMEOUT} ]]; do
  echo "Waiting for ${ASSETREPO_CRD} to come up in ${NAMESPACE}"
  count=$((count + 1))
  sleep 60
done
...
if [[ $count -eq 60 ]]; then
  ...
  echo "Timed out waiting for ${ASSETREPO_CRD} in ${NAMESPACE}"
  kubectl get all -n "${NAMESPACE}"
  exit 1
else
  echo "Found an instances of ${ASSETREPO_CRD} in a Running state in ${NAMESPACE}"
fi
